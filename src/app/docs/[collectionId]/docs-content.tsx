            ...
            <div className="p-4 space-y-1">
              ...
              {collection.requests.map((req) => (
                <a
                  ...
                  className={`flex items-center space-x-3 px-3 py-2 rounded-md text-sm transition-colors ${activeRequest === req.id
                    ? "bg-zinc-800 text-zinc-100 font-medium"
                    : "text-zinc-400 hover:bg-zinc-900/50 hover:text-zinc-200"
                    }`}
                >
                  <span
                    className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase w-12 text-center ${methodColors[req.method] || "bg-zinc-800 text-zinc-400"
                      }`}
                  >
                    {req.method}
                  </span>
...
function RequestSection({ request, isLast }: { request: Request; isLast: boolean }) {
  ...
  return (
    <section
      ...
      className={`px-10 py-20 flex flex-col xl:flex-row gap-12 ${!isLast ? "border-b border-zinc-800/50" : ""
        }`}
    >
      ...
      <div className="flex-1 xl:max-w-3xl space-y-8">
        <div>
          <div className="flex items-center space-x-3 mb-4">
            <Badge
              variant="outline"
              className={`text-sm font-bold tracking-wider px-2.5 py-1 uppercase rounded-md ${methodColors[request.method] || "bg-zinc-800 text-zinc-400"
                }`}
            >
              {request.method}
            </Badge>
